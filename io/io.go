...
func CreateWAV(dir string, name string, buf []int16, sampleSize int) {
	f, err := os.Create(fmt.Sprintf("%s/%s.wav", dir, name))
	if err != nil {
		panic(fmt.Sprintf("Could not create WAV file: %s", err))
	}

	writeWavHeader(f, sampleSize, audio.SampleRate, 16, 1)
